...
namespace QLKhachSan
{
    public partial class FormKhachHang : Form
    {
        ...
        string sql;
        string chuoiketnoi = @"Data Source=DESKTOP-GAPNNKK;Initial Catalog=QLKhachSan;Integrated Security=True";
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if (CheckValidate())
            {
                for (int i = 0; i < listThongThinKH.Items.Count; i++)
                {
                    if (listThongThinKH.Items[i].SubItems[].Add(docdulieu[6]).ToString().Equals(txtSodt.Text))
                    {
                    ...
                    if (listThongThinKH.Items[i].Cells[2].Value.ToString().Equals(txtCmnd.Text))
                    {
                        MessageBox.Show("Trùng cmnd");
                        return;
